...
public class QuizServiceImpl implements QuizService {
    ...
    public ResponseEntity<String> createQuiz(String category, int numQ, String title) {
        if(numQ<=0)
            throw new ApiNotFoundException("The quiz must include one or more questions." +
        ...
        if(questions.size()<numQ)
            throw new ApiNotFoundException("There are only "+questions.size()+" questions in this category");
    ...
    public ResponseEntity<List<QuestionWrapper>> getQuizQuestions(Integer id) {
        ...
        if(quiz.isEmpty())
            throw new ApiNotFoundException("not found any quiz with this id= "+id);
